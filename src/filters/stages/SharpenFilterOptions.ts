...
/** the sharpen stage's uniforms, named as its glsl declares them */
export const sharpenUniforms = (
  options: SharpenFilterOptions,
): Record<string, UniformData> => {
  const finalOptions = { ...defaultSharpenUniforms, ...options };
  return {
    uSharpenAmount: { value: finalOptions.amount, type: "f32" },
    uSharpenRadius: { value: finalOptions.radius, type: "f32" },
    uSharpenAsymmetry: { value: finalOptions.asymmetry, type: "f32" },
    uSharpenSignalBlur: { value: finalOptions.signalBlur, type: "f32" },
  };
};
